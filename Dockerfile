...
COPY target/gravitee-cloud-app-${CLOUD_APP_VERSION}.zip /tmp/
...
RUN apk update \
  ...
  && unzip /tmp/gravitee-cloud-app-${CLOUD_APP_VERSION}.zip -d /tmp/ \
  && apk del zip unzip netcat-openbsd wget \
  && cp -fr /tmp/gravitee-cloud-app-${CLOUD_APP_VERSION}/cloud-app/* ${WWW_TARGET} \
  && rm -rf /tmp/* \
